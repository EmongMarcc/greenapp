      ...
      <div class="flex divide-y divide-green-500">
        ...
        <div class="flex-grow">
          <div class="w-full text-center">
            <a class="flex flex-col" href="https://api.whatsapp.com/send?phone={{$team->number}}" target="_blank">
              <span class="font-mono text-gray-700 text-xs">{{$team->job}}</span>
              <span class="font-bold text-gray-700 text-xs">{{$team->name}}</span>
            </a>
          </div>
        </div>
        <span class="text-xs text-green-300">{{$team->status}}</span>
      </div>
            <?php endif; ?>
    <?php endforeach; ?>

    </div>
  <button type="button" @click="whatsapp = !whatsapp"  x-show="!whatsapp"  class="fixed bottom-24 right-0 bg-green-700 transition ease-in-out duration-150 m-3 hover:bg-green-800  rounded-full shadow-md w-8 h-8" name="button">
